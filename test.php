...
class AdminController extends BaseController {
    ...
    public function adminHandle(){
    	...
    	if($data['act'] == 'add'){
    		...
    		if(D('admin')->where("user_name='".$data['user_name']."'")->count()){
    			$this->error("此用户名已被注册，请更换",U('/sandy.php/Admin/admin_info'));
    		}else{
    			$r = D('admin')->add($data);
    		}
    	}
    	...
    	if($r){
    		$this->success("操作成功",U('/sandy.php/Admin/index'));
    	}else{
    		$this->error("操作失败",U('/sandy.php/Admin/index'));
    	}
    }
    ...
    public function login(){
        if(session('?admin_id') && session('admin_id')>0){
             $this->error("您已登录",U('/sandy.php/Index/index'));
        }
        ...
        if(IS_POST){
            ...
            if(!empty($condition['user_name']) && !empty($condition['password'])){
                ...
                if(is_array($admin_info)){
                    ...
                    $url = session('from_url') ? session('from_url') : U('/sandy.php/Index/index');
    ...
    public function logout(){
        session_unset();
        session_destroy();
        $this->success("退出成功",U('/sandy.php/Admin/login'));
    }
    ...
    public function roleSave(){
		...
		if($r){
			...
			$this->success("操作成功!",U('/sandy.php/Admin/role_info',array('role_id'=>$data['role_id'])));
		}else{
			$this->success("操作失败!",U('/sandy.php/Admin/role'));
		}
    }
	...
	public function supplierHandle()
	{
		...
		if ($data['act'] == 'add') {
			...
			if ($count) {
				$this->error("此供应商名称已被注册，请更换", U('/sandy.php/Admin/supplier_info'));
			} else {
		...
		if ($r !== false) {
			$this->success("操作成功", U('/sandy.php/Admin/supplier'));
		} else {
			$this->error("操作失败", U('/sandy.php/Admin/supplier'));
		}
	}
}
